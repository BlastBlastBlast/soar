...
class LocationForecastDataSource @Inject constructor(
    ...
    suspend fun fetchForecastDataResponse(lat: Double, lon: Double): Result<ForecastDataResponse> {
        return try {
            Log.i("LocationForecastDataSource", "Fetching LocationForecast data for (lat,lon) = ($lat,$lon)")
            ...
            Result.success(
                ...
                }.body<ForecastDataResponse>()
            )
                .also { Log.i("LocationForecastDataSource", "Successfully fetched LocationForecast data") }
